...
export default function SubscriptionPage() {
  ...
  return (
    <div className="max-w-3xl">
      ...
      <p className="mt-1 text-navy/60">Gestiona tu plan, método de pago y pagos.</p>

      {subscriptions === null && <p className="mt-6 text-sm text-navy/50">Cargando…</p>}

      {subscriptions?.length === 0 && (
        <div className="mt-6 rounded-card-md bg-white p-8 text-center shadow-sm">
          <p className="text-navy/70">Todavía no tienes una suscripción.</p>
          <Link href="/pricing" className="mt-3 inline-block font-semibold text-accent">
            Ver planes →
          </Link>
        </div>
      )}
      ...
      {current && (
        <>
          ...
          <div className="mt-6 rounded-card-md bg-white p-6 shadow-sm">
            <div className="flex items-start justify-between">
              <div>
                ...
                <h2 className="mt-2 text-xl font-semibold">{current.planName}</h2>
              </div>
              <span className={`rounded-pill px-3 py-1 text-xs font-semibold ${STATUS_STYLE[current.status]}`}>
                {STATUS_LABEL[current.status]}
              </span>
            </div>

            <div className="mt-4 grid grid-cols-2 gap-4 border-t border-navy/10 pt-4 text-sm sm:grid-cols-3">
              <div>
            ...
            {canCancel && (
              <button
                ...
                disabled={canceling}
                className="mt-5 rounded-pill border border-coral/30 px-4 py-2 text-sm font-semibold text-coral hover:bg-coral-soft disabled:opacity-50"
              >
                {canceling ? "Cancelando…" : "Cancelar suscripción"}
              </button>
            )}

            <Link href="/pricing" className="mt-5 ml-3 inline-block text-sm font-semibold text-accent">
              Cambiar de plan
            </Link>
          </div>
          ...
          <div className="mt-4 rounded-card-md bg-white p-6 shadow-sm">
            ...
            {payments === null && <p className="mt-3 text-sm text-navy/50">Cargando…</p>}

            {payments?.length === 0 && (
              <p className="mt-3 text-sm text-navy/60">Todavía no hay pagos registrados.</p>
            )}
                ...
                {payments.map((p) => (
                  <li key={p.id} className="flex items-center justify-between py-3 text-sm">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="font-semibold">{formatClp(p.amountClp)}</span>
                      <span
                        className={`rounded-pill px-2.5 py-1 text-[11px] font-semibold ${PAYMENT_STATUS_STYLE[p.status]}`}
                      >
